...
본 프로젝트는 API 서버를 중심으로 개발하며, 초기 단계에서는 프론트엔드(UI)는 고려하지 않는다.
...
직원(Employee) : 예약(Reservation)은 **N:1 관계**이다.
...
- BOOKED : 예약 완료
- CANCELED : 예약 취소
- DELETED : 예약 삭제 (DB에 물리적 삭제 하지 않음, 데이터 보관)
